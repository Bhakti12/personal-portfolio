...
export default function CaseStudyCard({
  ...
  const renderMockup = () => {
    switch (mockType) {
      case 'fleet':
        return (
          <div className="flex flex-col h-full font-mono text-[11px] leading-relaxed text-gray-300">
            {/* Header */}
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/[0.05]">
              <span className="text-gray-500 text-[10px]">database_partition.sql</span>
              <span className="text-[10px] text-matrix uppercase">postgresql</span>
            </div>
            {/* SQL Content */}
            <div className="flex-1 space-y-1 overflow-x-auto text-[10px]">
            ...
            <div className="mt-4 pt-3 border-t border-white/[0.05]">
              <div className="flex items-center justify-between pb-1 text-[10px] text-gray-500">
                <span>ingested_packet.json</span>
                <span className="h-2 w-2 rounded-full bg-matrix animate-ping" />
              </div>
      ...
      case 'healthcare':
        return (
          <div className="flex flex-col h-full font-mono text-[11px] leading-relaxed text-gray-300">
            {/* Header */}
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/[0.05]">
              <span className="text-gray-500 text-[10px]">fhir_patient_resource.json</span>
              <span className="text-[10px] text-matrix uppercase">aidbox / fhir</span>
            </div>
            {/* FHIR JSON */}
...
{`{
  ...
  "managingOrganization": {
    "reference": "Organization/senior-care-center"
              ...
              <div className="mt-3 pt-3 border-t border-white/[0.05]">
                <div className="text-[10px] text-gray-500 mb-1">kafka_producer.log</div>
                <div className="text-[9px] bg-black/40 p-2 rounded text-blue-400">
      ...
      case 'physio':
        return (
          <div className="flex flex-col h-full font-mono text-[11px] leading-relaxed text-gray-300">
            {/* Header */}
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/[0.05]">
              <span className="text-gray-500 text-[10px]">WorkloadMiddleware.php</span>
              <span className="text-[10px] text-matrix uppercase">laravel / php</span>
            </div>
            {/* Laravel Route & Middleware */}
            <div className="flex-1 space-y-1 overflow-x-auto text-[9px] sm:text-[10px]">
      ...
      case 'aviation':
        return (
          <div className="flex flex-col h-full font-mono text-[11px] leading-relaxed text-gray-300">
            {/* Header */}
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/[0.05]">
              <span className="text-gray-500 text-[10px]">aviation_sync_job.ts</span>
              <span className="text-[10px] text-matrix uppercase">node.js / ts</span>
            </div>
            {/* Aviation Sync */}
            <div className="flex-1 space-y-1 overflow-x-auto text-[10px]">
  ...
  return (
    ...
    >
      {/* Background glow for the active project card */}
      ...
      {/* Code Editor Mockup - Takes 5 Cols on desktop */}
      <div
        className={`lg:col-span-5 bg-dark-elevated border border-white/[0.08] rounded-2xl p-5 shadow-2xl relative overflow-hidden flex flex-col min-h-[300px] lg:min-h-0 ${
